const Regex = () =>
  [
    /\s*(select|SELECT)\s*([a-zA-Z,*_\s]+)(from|FROM)\s*([a-zA-Z]+)\s*$/g,
    /\s*(select|SELECT)\s*([a-zA-Z,*_\s]+)(from|FROM)\s*([a-zA-Z]+)\s*(where|WHERE)\s*(.*)(\s^\+|-|\*|\/|=|>|<|>=|&|\||%|!|\(|\))\s*(\d*)\s*$/g,
    ...
    /\s*(select|SELECT)\s*([a-zA-Z,*_\s]+)(from|FROM)\s*([a-zA-Z]+)\s*(left|LEFT)\s*(join|JOIN)\s*([a-zA-Z,*_\s]+)\s*(on)\s*([a-zA-Z,*_\s]+)\.([a-zA-Z,*_\s]+)\s*=\s*([a-zA-Z,*_\s]+)\.([a-zA-Z,*_\s]+)\s*$/g,
    /\s*(select|SELECT)\s*([a-zA-Z,*_\s]+)(from|FROM)\s*([a-zA-Z]+)\s*(left|LEFT)\s*(join|JOIN)\s*([a-zA-Z,*_\s]+)\s*(on)\s*([a-zA-Z,*_\s]+)\.([a-zA-Z,*_\s]+)\s*=\s*([a-zA-Z,*_\s]+)\.([a-zA-Z,*_\s]+)\s*$/g,
